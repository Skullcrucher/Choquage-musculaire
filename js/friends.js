  ...
  return `
    <div class="list-row" style="cursor:default; gap:10px;">
      <div style="display:flex; align-items:center; gap:10px; min-width:0; cursor:pointer;" ${uid ? `data-profile="${esc(uid)}"` : ""}>
        ${avatar(profile)}
        <div class="list-row-title" style="overflow:hidden; text-overflow:ellipsis;">${esc(profile?.display_name || t("Utilisateur"))}</div>
      </div>
...
export async function renderFriends(container) {
  ...
  container.innerHTML = `
    ...
    <div class="card">
      ...
      <input id="friend-search" type="search" placeholder="${t("Rechercher un pseudo (2 lettres min.)")}">
      <div id="friend-results"></div>
    </div>
    ${incoming.length ? `<div class="card">
      <div class="card-title">${t("Demandes reçues")}</div>
      ${incoming.map(f => personRow({ uid: f.other_uid, ...profiles[f.other_uid] }, `
        <button class="btn btn-sm btn-primary" data-accept="${esc(f.id)}">${t("Accepter")}</button>
        <button class="btn btn-sm btn-secondary" data-remove="${esc(f.id)}">${t("Refuser")}</button>`)).join("")}
    </div>` : ""}
    <div class="card">
      <div class="card-title">${t("Mes amis ({n})", { n: friends.length })}</div>
      ${friends.length ? friends.map(f => personRow({ uid: f.other_uid, ...profiles[f.other_uid] }, `<button class="btn btn-sm btn-secondary" data-remove="${esc(f.id)}" data-confirm="1">${t("Retirer")}</button>`)).join("")
        : `<p class="muted" style="margin:0;">${t("Pas encore d'amis. Cherche leur pseudo ci-dessus.")}</p>`}
    </div>
    ${outgoing.length ? `<div class="card">
      <div class="card-title">${t("Demandes envoyées")}</div>
  ...
  container.querySelectorAll("[data-accept]").forEach(btn => btn.onclick = async () => {
    ...
    try {
      ...
      toast(t("Ami ajouté"), 2200, { horns: true });
    } catch (err) {
      console.error("[Skullcrusher] Erreur acceptation ami", err);
      toast(t("Impossible d'accepter la demande"));
    }
    await refresh();
  });
  container.querySelectorAll("[data-remove]").forEach(btn => btn.onclick = async () => {
    ...
    if (!f) return;
    if (btn.dataset.confirm && !confirm(t("Retirer {name} de tes amis ? Tes routines partagées avec lui ne lui seront plus visibles.", { name: profiles[f.other_uid]?.display_name || t("cet ami") }))) return;
    btn.disabled = true;
    try {
  ...
  searchInput.addEventListener("input", debounce(async () => {
    ...
    resultsEl.innerHTML = found.length === 0
      ? `<p class="muted" style="margin:10px 0 0;">${t("Personne avec ce pseudo.")}</p>`
      : found.map(p => {
          const f = knownUids.get(p.uid);
          const action = !f ? `<button class="btn btn-sm btn-primary" data-request="${esc(p.uid)}">${t("Ajouter")}</button>`
            : f.status === "accepted" ? `<span class="muted">${t("Ami ✓")}</span>`
            : f.incoming ? `<button class="btn btn-sm btn-primary" data-accept-search="${esc(f.id)}">${t("Accepter")}</button>`
            : `<span class="muted">${t("Demande envoyée")}</span>`;
    ...
    resultsEl.querySelectorAll("[data-request]").forEach(btn => btn.onclick = async () => {
      ...
      try {
        ...
        toast(t("Demande envoyée"));
        await refresh();
      } catch (err) {
        console.error("[Skullcrusher] Erreur demande d'ami", err);
        toast(t("Demande impossible (déjà envoyée ?)"));
        btn.disabled = false;
      }
    });
